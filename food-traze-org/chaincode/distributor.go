...
type Asset3 struct {
	...
	Size           int        `json:"Size"`
	Data           Attribute3 `json:"Attributes"`
	Test           Lab3       `json:"Labtest"`
	Val            string
}

type Attribute3 struct {
...
// 		err = setAssetStateBasedEndorsement3(ctx, asset.ID, endorsingOrgs)
// 		if err != nil {
...
func (s *SmartContract) CreateAssetDistributor(ctx contractapi.TransactionContextInterface, id string, color string, size int, owner string, appraisedValue int, data string, lab string) error {
	...
	exists, err := s.AssetExistsDistributor(ctx, id)
	if err != nil {
		return err
	}
	if exists {
...
func (s *SmartContract) ReadAssetDistributor(ctx contractapi.TransactionContextInterface, id string) (*Asset3, error) {
...
func (s *SmartContract) UpdateAssetDistributor(ctx contractapi.TransactionContextInterface, id string, color string, size int, owner string, appraisedValue int) error {
	exists, err := s.AssetExistsDistributor(ctx, id)
	if err != nil {
		return err
	}
...
func (s *SmartContract) DeleteAssetDistributor(ctx contractapi.TransactionContextInterface, id string) error {
	exists, err := s.AssetExistsDistributor(ctx, id)
	if err != nil {
		return err
	}
...
func (s *SmartContract) AssetExistsDistributor(ctx contractapi.TransactionContextInterface, id string) (bool, error) {
	assetJSON, err := ctx.GetStub().GetState(id)
	if err != nil {
...
func (s *SmartContract) TransferAssetDistributor(ctx contractapi.TransactionContextInterface, id string, newOwner string) (string, error) {
	asset, err := s.ReadAssetDistributor(ctx, id)
	if err != nil {
		return "", err
	}
...
func (s *SmartContract) GetAllAssetsDistributor(ctx contractapi.TransactionContextInterface) ([]*Asset3, error) {
...
func setAssetStateBasedEndorsement3(ctx contractapi.TransactionContextInterface, assetID string, orgsToEndorse []string) error {
