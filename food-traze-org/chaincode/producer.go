...
// GetQrTraceabilityEventsByBatchId returns all assets found in world state
func (s *SmartContract) GetQrTraceabilityEventsByBatchId(ctx contractapi.TransactionContextInterface, cropId string, label string) (map[string]interface{}, error) {
	assets := make(map[string]interface{})
	if label == "Batch" {
		// --------------------   Get ProductData   -----------------------
		// filters := fmt.Sprintf("{\"selector\":{\"Data.BatchesId\":\"%s\",\"DocType\":\"%s\"}}", cropId, "Batches")
		// filters := fmt.Sprintf(`{"selector": {"DocType": "Batches","Data": {"BatchesId": {"$elemMatch": {"BatchId": "%s"}}}}}`, cropId)
		filters := fmt.Sprintf(`{"selector": {"DocType": "Event","Data.Batch Number": "%s"}}`, cropId)
		...
		if !found {
			// Handle when cropId not found in Event ledger
			return map[string]interface{}{
				...
				"Ingredient": nil,
				"Events":     nil,
				"Message":    fmt.Sprintf("No Event data found for BatchId: %s", cropId),
			}, nil
		}
		assets["BatchData"] = asset0
		...
		dataMap, ok := asset0["Data"].(map[string]interface{})
		if !ok {
		...
		// // 1. HarvestBatches
		// if hb, exists := dataMap["HarvestBatches"]; exists {
		// 	if batches, ok := hb.([]interface{}); ok {
		// 		for _, item := range batches {
		// 			if batch, ok := item.(map[string]interface{}); ok {
		// 				batchIds = append(batchIds, batch["Ftlc"])
		// 			}
		// 		}
		// 	}
		// }
		...
		// // 2. DryingBatches
		// if db, exists := dataMap["DryingBatches"]; exists {
		// 	if batches, ok := db.([]interface{}); ok {
		// 		for _, item := range batches {
		// 			if batch, ok := item.(map[string]interface{}); ok {
		// 				batchIds = append(batchIds, batch["Ftlc"])
		// 			}
		...
		// 	}
		// }
		// Choose a multiple batches
		if cb, exists := dataMap["ChooseBatches"]; exists {
			if batches, ok := cb.([]interface{}); ok {
				for _, item := range batches {
					if batch, ok := item.(map[string]interface{}); ok {
						batchIds = append(batchIds, batch["Ftlc"])
					}
		...
		for _, ids := range batchIds {
			batchId := ids.(string)
			filter1 := fmt.Sprintf("{\"selector\":{\"FTLCID\":\"%s\",\"DocType\":\"%s\"}}", batchId, "Event")
		...
		// --------------------   Get Ingredient Events Data Producer   -----------------------
		// if len(ingredient) != 0 {
...
// GetQrTraceabilityEventsStoryOrg returns all assets found in world state
func (s *SmartContract) GetQrTraceabilityEventsStoryOrg(ctx contractapi.TransactionContextInterface, cropId string, label string) (map[string]interface{}, error) {
	assets := make(map[string]interface{})
	if label == "Batch" {
		// --------------------   Get ProductData   -----------------------
		// filters := fmt.Sprintf("{\"selector\":{\"Data.BatchesId\":\"%s\",\"DocType\":\"%s\"}}", cropId, "Batches")
...
// ReadAsset returns the asset stored in the world state with given id for Asset Transfer.
func (s *SmartContract) TrazeKdesTransferOrg(ctx contractapi.TransactionContextInterface, id string, typeOrg string, transferId string, currentOwner string, previousOwner string, remarks string) (bool, error) {
	assetJSON, err := ctx.GetStub().GetState(id)
	if err != nil {
		return false, fmt.Errorf("failed to read data from world state: %v", err)
	}
	...
	var curOwner map[string]interface{}
	if err1 := json.Unmarshal([]byte(currentOwner), &curOwner); err1 != nil {
		// fmt.Println("Error parsing JSON1:", err1)
		return false, fmt.Errorf("the current owner data error %v", err1)
	}
	var prevOwner map[string]interface{}
	if err2 := json.Unmarshal([]byte(previousOwner), &prevOwner); err2 != nil {
		// fmt.Println("Error parsing JSON1:", err1)
		return false, fmt.Errorf("the previous owner data error %v", err2)
	}
	kdes["AliasOrgName"] = typeOrg
	kdes["Status"] = "Transferred"
	userId, ok := curOwner["UserId"].(float64)
	if !ok {
		return false, fmt.Errorf("invalid UserId")
	}
	kdes["UserId"] = int(userId)
	kdes["UserName"] = curOwner["UserName"].(string)
	kdes["TenantId"] = curOwner["TenantId"].(string)
	// kdes["ToParentId"] = ownerId
	// kdes["FromUserName"] = fromUserName
	// kdes["FromUserId"] = fromUserId
	// kdes["FarmName"] = toOwnerName
	kdes["IsAccept"] = 1
	kdes["CurrentOwner"] = curOwner
	kdes["PreviousOwner"] = prevOwner
	kdes["TransferId"] = transferId
	kdes["Remarks"] = remarks
	// kdes["UserLocation"] = userLocation
	// kdes["FarmLocation"] = ownerLocation
	assetJSON2, err4 := json.Marshal(kdes)
	if err4 != nil {
	...
	orgMap := map[string]string{
		"Food Producer":  "Org1MSP",
		"Food Processor": "Org2MSP",
		"Exporter":       "Org4MSP",
		"Importer":       "Org5MSP",
	}

	msp, ok := orgMap[typeOrg]
	if !ok {
		return false, fmt.Errorf("unknown organization type")
	}

	err = setAssetStateBasedEndorsement(ctx, id, []string{msp})
	if err != nil {
		return false, fmt.Errorf("failed setting SBE: %v", err)
	}
	return true, nil
}

// ReadAsset returns the asset stored in the world state with given id.
func (s *SmartContract) TransferedIsAccept(ctx contractapi.TransactionContextInterface, id string, value int, reason string) (bool, error) {
	assetJSON, err := ctx.GetStub().GetState(id)
	if err != nil {
		return false, fmt.Errorf("failed to read data from world state: %v", err)
	}
	...
	kdes["IsAccept"] = value
	if value == 3 {
		kdes["RejectReason"] = reason
	}
	assetJSON2, err4 := json.Marshal(kdes)
	if err4 != nil {
		return false, fmt.Errorf("the asset json %s already exists", assetJSON2)
	}
	err = ctx.GetStub().PutState(id, assetJSON2)
	if err != nil {
		return false, fmt.Errorf("failed to update asset: %v", err)
	}
...
// ReadAsset returns the asset stored in the world state with given id.
func (s *SmartContract) UpdateAreaFieldInLedger(ctx contractapi.TransactionContextInterface, id string, value string) (bool, error) {
	assetJSON, err := ctx.GetStub().GetState(id)
	if err != nil {
		return false, fmt.Errorf("failed to read data from world state: %v", err)
	}
