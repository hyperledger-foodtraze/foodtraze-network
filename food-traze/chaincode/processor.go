...
func (s *SmartContract) CreateAssetProcessor(ctx contractapi.TransactionContextInterface, id string, color string, size int, owner string, appraisedValue int, data string, lab string) error {
	...
	// }
	exists, err := s.AssetExistsProcessor(ctx, id)
	if err != nil {
		return err
	}
	if exists {
	...
	// Changes the endorsement policy to the new owner org
	endorsingOrgs := []string{"Org1MSP"}
	err = setAssetStateBasedEndorsement2(ctx, asset.AssetID, endorsingOrgs)
	if err != nil {
		return fmt.Errorf("failed setting state based endorsement for new owner: %v", err)
	}
...
func (s *SmartContract) AssetTransferApprove(ctx contractapi.TransactionContextInterface, id string, status string, owner string) error {
	exists, err := s.AssetExistsProcessor(ctx, id)
	if err != nil {
		return err
	}
	...
	asset, err := s.ReadAssetProcessor(ctx, id)
	if err != nil {
		return err
	}
	if asset.Status == "Pending" {
		...
		return fmt.Errorf("the asset %s is not pending", id)
	}

	// Changes the endorsement policy to the new owner org
	endorsingOrgs := []string{owner}
	err = setAssetStateBasedEndorsement2(ctx, asset.AssetID, endorsingOrgs)
	if err != nil {
		return fmt.Errorf("failed setting state based endorsement for new owner: %v", err)
	}
...
// UpdateAsset updates an existing asset in the world state with provided parameters.
func (s *SmartContract) AssetTransferReject(ctx contractapi.TransactionContextInterface, id string, status string) error {
	exists, err := s.AssetExistsProcessor(ctx, id)
	if err != nil {
		return err
	}
	...
	if asset.Status == "Pending" {
		...
		return fmt.Errorf("the asset %s already approved", id)
	}
...
func (s *SmartContract) ReadAssetProcessor(ctx contractapi.TransactionContextInterface, id string) (*Asset2, error) {
...
// UpdateAsset updates an existing asset in the world state with provided parameters.
func (s *SmartContract) UpdateAssetProcessor(ctx contractapi.TransactionContextInterface, id string, color string, size int, owner string, appraisedValue int) error {
	exists, err := s.AssetExistsProcessor(ctx, id)
	if err != nil {
		return err
	}
...
func (s *SmartContract) DeleteAssetProcessor(ctx contractapi.TransactionContextInterface, id string) error {
	exists, err := s.AssetExistsProcessor(ctx, id)
	if err != nil {
		return err
	}
...
func (s *SmartContract) AssetExistsProcessor(ctx contractapi.TransactionContextInterface, id string) (bool, error) {
	assetJSON, err := ctx.GetStub().GetState(id)
	if err != nil {
...
func (s *SmartContract) TransferAssetProcessor(ctx contractapi.TransactionContextInterface, id string, newOwner string) (string, error) {
	asset, err := s.ReadAssetProcessor(ctx, id)
	if err != nil {
		return "", err
	}
...
func (s *SmartContract) GetAllAssetsProcessor(ctx contractapi.TransactionContextInterface) ([]*Asset2, error) {
